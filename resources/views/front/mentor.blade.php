...
@section('lang')
    @foreach (config('app.languages') as $locale)
        <a href="{{ route('mentor', $locale) }}" @if (app()->getLocale() == $locale) style="font-weight: bold; text-decoration: underline" @endif class="text-link-white-2">{{ strtoupper($locale) }}</a>
    @endforeach
@endsection
...
<section class="wow fadeIn bg-light-gray">
    <div class="container">
        <form action="{{route('mentor.send',app()->getLocale())}}" method="POST">
            @csrf
